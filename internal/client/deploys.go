...
func (c *Client) ListDeployments(ctx context.Context, orgID, appID string) ([]Deployment, error) {
	var resp []Deployment
	if err := c.get(ctx, "/api/v1/orgs/"+orgID+"/apps/"+appID+"/deployments", &resp); err != nil {
		return nil, err
	}
	return resp, nil
}
...
func (c *Client) TriggerDeployment(ctx context.Context, orgID, appID, releaseTag, environmentID string) (*Deployment, error) {
	...
	var resp Deployment
	if err := c.post(ctx, "/api/v1/orgs/"+orgID+"/apps/"+appID+"/deployments", req, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
...
func (c *Client) GetDeployment(ctx context.Context, orgID, appID, deployID string) (*Deployment, error) {
	var resp Deployment
	if err := c.get(ctx, "/api/v1/orgs/"+orgID+"/apps/"+appID+"/deployments/"+deployID, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
