...
func TestClientSetsUserAgent(t *testing.T) {
	...
	if !strings.HasPrefix(gotUA, "septiembre-cli/") {
		t.Errorf("User-Agent = %q, want prefix 'septiembre-cli/'", gotUA)
	}
}
...
func TestBaseURLJoining_NoDoubleSlash(t *testing.T) {
	t.Parallel()
	...
	// Trailing slash on base URL must not produce //api/v1/orgs.
	c := client.New(srv.URL+"/", testToken, client.WithRetryDelay(func(int) time.Duration { return 0 }))
	_, _ = c.ListOrgs(context.Background())
...
// Scenario B-02a: 401 must carry a guidance message that mentions SEPTIEMBRE_TOKEN.
func TestErrorParsing_401_GivesGuidanceMessage(t *testing.T) {
	...
	if !strings.Contains(apiErr.Message, "SEPTIEMBRE_TOKEN") {
		t.Errorf("401 message does not mention SEPTIEMBRE_TOKEN: %q", apiErr.Message)
	}
}
