...
// CreateTokenResponse is returned by POST /api/v1/auth/tokens.
...
func (c *Client) CreateToken(ctx context.Context, name string, expiresAt *time.Time) (*CreateTokenResponse, error) {
	...
	if err := c.post(ctx, "/api/v1/auth/tokens", req, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
...
func (c *Client) ListTokens(ctx context.Context) ([]Token, error) {
	var resp []Token
	if err := c.get(ctx, "/api/v1/auth/tokens", &resp); err != nil {
		return nil, err
	}
	return resp, nil
}
...
func (c *Client) RevokeToken(ctx context.Context, tokenID string) error {
	return c.delete(ctx, "/api/v1/auth/tokens/"+tokenID)
}
