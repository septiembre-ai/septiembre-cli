...
func htmlGraphData(graph Graph) htmlGraph {
	...
	for _, id := range ids {
		...
		c := graph.Churn[id]
		viewNodes = append(viewNodes, htmlNode{ID: id, Status: s, Added: c.Added, Deleted: c.Deleted})
	}
	...
	return htmlGraph{Base: graph.Base, Nodes: viewNodes, Edges: edges, Diffs: diffs, Release: graph.Release, Changelog: graph.Changelog, Checklist: graph.Checklist}
}
...
(() => {
  ...
  if (graph.release) {
    document.querySelector('.title').textContent = 'Release ' + graph.release;
    ...
    document.getElementById('meta').textContent = 'since ' + (graph.base || 'start') + ' · ' + changedCount + ' files · ' + commitCount + ' commits';
  } else {
  ...
  for (const item of (graph.checklist || [])) {
    ...
    row.className = 'check' + (item.ok ? ' ok' : '');
  ...
  function tick() {
    ...
    nodeEls.forEach((group, i) => group.setAttribute('transform', 'translate(' + nodes[i].x + ' ' + nodes[i].y + ')'));
  }
  ...
  function showDiff(node) {
    ...
    for (const line of text.split('\n')) {
      ...
      if (kind === 'meta' || kind === 'hunk') {
        ...
        const prefix = (kind === 'add' || kind === 'del') ? line.charAt(0) : '';
        div.innerHTML = escapeHtml(prefix) + highlight(line.slice(prefix.length), spec);
      }
      diffBody.appendChild(div);
    }
    diffBody.scrollTop = 0;
  }
  let activeLi = null, activeEl = null;
  function selectNode(node, el, li) {
    if (activeEl) activeEl.classList.remove('selected');
    if (activeLi) activeLi.classList.remove('active');
    activeEl = el; activeLi = li;
