...
import (
	...
	"github.com/septiembre-ai/septiembre-cli/internal/changes"
...
// NewChangesCmd returns the `septiembre changes` command.
...
func writeChangesHTML(graph changes.Graph) (string, error) {
	file, err := os.CreateTemp("", "septiembre-changes-*.html")
	if err != nil {
		return "", err
	}
	path := file.Name()
