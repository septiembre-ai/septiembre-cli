...
func TestServicesKVSTablesCreate_JSONOutput(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/api/v1/orgs", orgsHandler("acme", "org-1"))
	mux.HandleFunc("/api/v1/orgs/org-1/apps/app-1/services/kvs/tables", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			t.Errorf("method = %s, want POST", r.Method)
		}
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusCreated)
		_, _ = w.Write([]byte(`{"table":{"id":"table-1","name":"sessions","status":"active","kvs_url":"https://kvs.septiembre.ai","env_var_name":"SEPTIEMBRE_KVS_TOKEN_SESSIONS","token_last_four":"cret","minute_limit":300,"created_at":"2026-06-20T00:00:00Z","updated_at":"2026-06-20T00:00:00Z"},"token":"skvs_table_secret"}`))
	})
	srv := newTestServer(t, mux)
	outBuf, _, exec := newTestRoot(t, srv)
