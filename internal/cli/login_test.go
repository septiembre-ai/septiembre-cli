...
// cloudAPIStub returns an httptest server backing POST /api/v1/auth/tokens
// and GET /api/v1/auth/me, asserting the bearer token used for each leg.
...
func setLoginEnv(t *testing.T, cognitoDomain, clientID, apiURL string) {
	t.Helper()
	t.Setenv("SEPTIEMBRE_COGNITO_DOMAIN", cognitoDomain)
	t.Setenv("SEPTIEMBRE_COGNITO_CLIENT_ID", clientID)
	t.Setenv("SEPTIEMBRE_API_URL", apiURL)
