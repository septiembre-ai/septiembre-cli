...
// archives are named septiembre_<version>_<os>_<arch>.<ext> where <version> is
// the git tag without its leading "v".
const (
	ghOwner    = "septiembre-ai"
	ghRepo     = "septiembre-cli"
	binaryName = "septiembre"
)
...
// NewUpgradeCmd returns the `septiembre upgrade` command.
//
...
// `brew upgrade septiembre`, so the package manager's version tracking stays
// authoritative.
func NewUpgradeCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "upgrade",
		Short: "Upgrade the CLI to the latest released version",
		Long: `Upgrade the septiembre CLI to the latest GitHub release.
...
BEHAVIOUR
  ...
  'brew upgrade --cask septiembre' so Homebrew keeps managing the version.
...
func runUpgrade(cmd *cobra.Command, checkOnly bool) error {
	...
	if isHomebrewManaged(execPath) {
		_ = r.Render(map[string]string{
			...
			"hint":    "run: brew upgrade --cask septiembre",
		})
		return nil
	}
...
func httpGet(ctx context.Context, url string, timeout time.Duration) ([]byte, error) {
	...
	req.Header.Set("User-Agent", "septiembre-cli/"+version.Version)
...
// extractBinary pulls the septiembre binary out of a release archive. Unix
...
func replaceBinary(dst string, data []byte) error {
	...
	tmp, err := os.CreateTemp(dir, ".septiembre-upgrade-*")
	if err != nil {
