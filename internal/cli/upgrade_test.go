...
func TestAssetName(t *testing.T) {
	tests := []struct {
		...
		{"darwin arm64 tarball", "0.2.1", "darwin", "arm64", "septiembre_0.2.1_darwin_arm64.tar.gz"},
		{"linux amd64 tarball", "0.2.1", "linux", "amd64", "septiembre_0.2.1_linux_amd64.tar.gz"},
		{"windows amd64 zip", "0.2.1", "windows", "amd64", "septiembre_0.2.1_windows_amd64.zip"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestChecksumFor(t *testing.T) {
	sums := "abc123  septiembre_0.2.1_darwin_arm64.tar.gz\n" +
		"def456  septiembre_0.2.1_linux_amd64.tar.gz\n"

	t.Run("found", func(t *testing.T) {
		got, err := checksumFor(sums, "septiembre_0.2.1_linux_amd64.tar.gz")
		if err != nil {
	...
	t.Run("missing entry errors", func(t *testing.T) {
		if _, err := checksumFor(sums, "septiembre_9.9.9_linux_amd64.tar.gz"); err == nil {
...
func TestIsHomebrewManaged(t *testing.T) {
	tests := []struct {
		...
		{"caskroom", "/opt/homebrew/Caskroom/septiembre/0.2.1/septiembre", true},
		{"cellar", "/usr/local/Cellar/septiembre/0.2.1/bin/septiembre", true},
		{"standalone usr local bin", "/usr/local/bin/septiembre", false},
		{"home bin", "/home/dev/.local/bin/septiembre", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestReplaceBinary(t *testing.T) {
	dir := t.TempDir()
	dst := filepath.Join(dir, "septiembre")
	...
	entries, err := filepath.Glob(filepath.Join(dir, ".septiembre-upgrade-*"))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 0 {
